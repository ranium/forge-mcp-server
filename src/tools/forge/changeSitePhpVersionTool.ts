...
const paramsSchema = {
  ...
  phpVersion: z
    .string()
    .describe(
      'The PHP version to set for the site. The client MUST validate this value against the available PHP versions from listPhpVersionsTool before passing it, as this is plain text from the user.'
    ),
  confirmationId: z
    .string()
    .describe(
      'This confirmationId must be obtained from confirmChangeSitePhpVersionTool after explicit user confirmation. If an invalid or mismatched confirmationId is provided, the PHP version change will be rejected.'
    ),
}

const paramsZodObject = z.object(paramsSchema)

export const changeSitePhpVersionTool: ForgeToolDefinition<
  typeof paramsSchema
> = {
