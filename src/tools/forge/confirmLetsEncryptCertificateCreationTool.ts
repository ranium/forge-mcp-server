...
const paramsSchema = {
    ...
    domains: z.array(z.string()).describe("Array of domain names for which to obtain the Let's Encrypt certificate."),
    dns_provider: z.object({
    ...
    }).optional().describe("DNS provider configuration for wildcard certificates.")
};

const paramsZodObject = z.object(paramsSchema);
type Params = z.infer<typeof paramsZodObject>;

export const createLetsEncryptCertificateConfirmationStore = createConfirmationStore<Params>();

export const confirmLetsEncryptCertificateCreationTool: ForgeToolDefinition<typeof paramsSchema> = {
    name: "confirm_lets_encrypt_certificate_creation",
    description: "Confirms the request to create a Let's Encrypt SSL certificate for a site and returns a summary for user confirmation. This tool does not perform the operation, but returns a summary and expects the client to handle the confirmation logic.",
