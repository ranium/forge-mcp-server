...
const paramsSchema = {
  ...
  ]).describe("The ID of the server to check Laravel Scheduler status for (string, number, or { value: string|number })"),
  siteId: z.union([
    z.string(),
    z.number(),
    z.object({ value: z.union([z.string(), z.number()]) }).transform(obj => obj.value)
  ]).describe("The ID of the site to check Laravel Scheduler status for (string, number, or { value: string|number })"),
};
